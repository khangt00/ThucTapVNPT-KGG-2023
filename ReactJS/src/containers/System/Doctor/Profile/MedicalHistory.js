...
import '../../../Profile/scss/MedicalHistory.scss'
import { FormattedMessage } from 'react-intl'

import { useHistory } from 'react-router-dom'
import { useDispatch, useSelector } from 'react-redux'

import EditProfileModal from '../../../Profile/resources/EditProfileModal'
...
import { filterDoctorHistories, filterHistoriesPatient } from '../../../../services/userService'
...
export default function DoctorMedicalHistory() {
  ...
  const handleFilterHistoryByDateApply = async () => {
    ...
    let res = await filterDoctorHistories(data)
  ...
  const handleGetValueUnit = (unitKey) => {
    let found = units.find((item) => item.key === unitKey)
    if (found) {
      if (language === 'vi') return found.valueVi
      else return found.valueEn
    }
  }
      ...
      <div>
        ...
        <div class="row">
          ...
          <div class="col-12 mt-10">
            <button onClick={() => handleFilterHistoryByDateApply()} type="button" class="btn btn-primary mr-5">
            ...
            <table class="table table-hover mt-45">
              <thead>
                <tr>
                  ...
                  <th scope="col" class="text-center">
                    <FormattedMessage id="medical-history.patient" />
                  </th>
                  <th scope="col" class="text-center">
                    ...
                    <tr key={index}>
                      ...
                      <td class="text-center">
                        <div class="text-primary">
                          {item.patientDataHistory.lastName} {item.patientDataHistory.firstName}
                        </div>
                        {/* <div
                          class="pointer text-primary"
                          onClick={() =>
                            history.push(`/detail-specialty/${item.patientDataHistory.Doctor_Infor.specialtyId}`)
                          }
                        >
                          {item.patientDataHistory.Doctor_Infor.specialtyData.name}
                        </div>
                        <div
                          class="pointer text-primary"
                          onClick={() => history.push(`/detail-clinic/${item.patientDataHistory.Doctor_Infor.clinicId}`)}
                        >
                          {item.patientDataHistory.Doctor_Infor.clinicData.name}
                        </div> */}
                      </td>
                      <td>
                        <ul class="list-group" style={{ overflowY: 'scroll', maxHeight: '150px' }}>
                          {drugs.map((item, index) => {
                            return (
                              <li key={index} class="list-group-item list-group-item-action">
                      ...
                      <td class="text-center">
                        <div className="text-center pointer text-primary" onClick={() => openPreviewImage(item)}>
